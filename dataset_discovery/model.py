...
from torch_geometric.nn import GCNConv, TransformerConv, global_max_pool
...
def test(model: GCN, loader: DataLoader) -> float:
    """Evaluate accuracy of model by percentage of
    molecules classsified correctly in all classes.
    ...
    for data in loader:
        ...
        out[out >= 0.5] = 1
        out[out < 0.5] = 0
        correct += torch.all(out == torch.Tensor(data.y), axis=1).sum()
